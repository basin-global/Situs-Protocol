...
describe("BasinTLDFactory", function () {
    ...
    describe("Create TLD", function () {
        ...
        it("should create a new valid TLD through ownerCreateTld()", async function () {
            ...
            await basinTLDFactory.ownerCreateTld(tldName, tldSymbol, owner.address, domainPrice, false);
        });
        ...
        it("should fail to create a new valid TLD if user is not owner", async function () {
            ...
            await expect(
                basinTLDFactory.connect(otherAccount).ownerCreateTld(
                    tldName,
                    tldSymbol,
        ...
        it("should fail to create a new valid TLD if more than 1 dot in the name", async function () {
            const { basinTLDFactory, owner } = await loadFixture(deployBasinTLDFactoryFixture);
            await expect(
                basinTLDFactory.ownerCreateTld(
                    ".ba.sin", // Invalid TLD
                    tldSymbol,
                    owner.address,
                    domainPrice,
        ...
        it("should fail to create a new valid TLD if TLD already exists", async function () {
            const { basinTLDFactory, owner } = await loadFixture(deployBasinTLDFactoryFixture);
            // create a valid TLD
            await expect(basinTLDFactory.ownerCreateTld(tldName, tldSymbol, owner.address, domainPrice, false)).to.emit(
                basinTLDFactory,
                "TldCreated",
            );

            // try to create another TLD with the same name
            await expect(basinTLDFactory.ownerCreateTld(tldName, tldSymbol, owner.address, domainPrice, false)).to.be.revertedWith(
        ...
        it("non-owner cannot change the TLD price", async function () {
            ...
            await expect(basinTLDFactory.connect(otherAccount).changePrice(ethers.parseUnits("2", "ether"))).to.be.revertedWith(
        ...
        it("non-owner cannot change max length for a TLD name", async function () {
            ...
            await expect(basinTLDFactory.connect(otherAccount).changeNameMaxLength(60)).to.be.revertedWith(
                "Ownable: caller is not the owner",
            );
        ...
        it("non-owner cannot add a new forbidden domain", async function () {
            ...
            // fail if sender is not owner
            await expect(basinForbiddenTLDs.connect(otherAccount).ownerAddForbiddenTld(".io")).to.be.revertedWith(
                "Ownable: caller is not the owner",
            );
        ...
        it("non-owner cannot remove a forbidden domain", async function () {
            ...
            await expect(basinForbiddenTLDs.connect(otherAccount).removeForbiddenTld(".net")).to.be.revertedWith(
                "Ownable: caller is not the owner",
            );
