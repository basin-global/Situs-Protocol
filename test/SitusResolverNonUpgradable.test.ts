...
describe("SitusResolverNonUpgradable", function () {
    ...
    async function deploySitusResolverNonUpgradableFixture() {
        ...
        return { situsResolverNonUpgradable, situsTLDFactory1, situsTLDFactory2, situsForbiddenTLDs, signer, user1, user2, tldContract1, tldContract2, tldContract3 };
    }
    ...
    describe("Multiple Factory Test", function () {
        it('adds and removes a factory address (only owner)', async function () {
            const { situsResolverNonUpgradable, situsTLDFactory1, situsTLDFactory2, situsForbiddenTLDs, signer, user1, user2, tldContract1, tldContract2, tldContract3 } = await loadFixture(deploySitusResolverNonUpgradableFixture);
            ...
            // user1 tries to add Factory2 address, should revert
            await expect(situsResolverNonUpgradable.connect(user1).addFactoryAddress(
              situsTLDFactory2.getAddress()
            ...
            await expect(situsResolverNonUpgradable.connect(user1).removeFactoryAddress(
              0
            )).to.be.revertedWith('Ownable: caller is not the owner');
          });
          ...
          it('fetches a default domain for a user address', async function () {
            ...
            const defaultDomainViaResolverBefore = await situsResolverNonUpgradable.getDefaultDomain(user1.address, tldName1);
            ...
            const defaultDomainViaResolverAfter = await situsResolverNonUpgradable.getDefaultDomain(user1.address, tldName1);
            expect(defaultDomainViaResolverAfter).to.equal(domainName);
        
            // check non-existing default domain via Resolver
            const defaultDomainViaResolverNonExisting = await situsResolverNonUpgradable.getDefaultDomain(user2.address, tldName1);
            ...
            const domainDomainViaResolverDeprecated = await situsResolverNonUpgradable.getDomainData(user1.address, tldName1);
          ...
          it('fetches a list of default domains for a user address across all TLDs', async function () {
            ...
            // check default domain via Resolver before
            const defaultDomainViaResolverBefore = await situsResolverNonUpgradable.getDefaultDomains(user1.address);
            expect(defaultDomainViaResolverBefore).to.equal("");
            ...
            // check default domain via Resolver after
            const defaultDomainViaResolverAfter = await situsResolverNonUpgradable.getDefaultDomains(user1.address);
            expect(defaultDomainViaResolverAfter).to.equal(domainName + tldName1 + " " + domainName + tldName3);
        
            // check non-existing default domain via Resolver
            const defaultDomainViaResolverNonExisting = await situsResolverNonUpgradable.getDefaultDomains(user2.address);
            expect(defaultDomainViaResolverNonExisting).to.equal("");
            ...
            const defaultDomainViaResolverAfter2 = await situsResolverNonUpgradable.getDefaultDomains(user1.address);
          ...
          it('fetches a single users default domain name, the first that comes', async function () {
            ...
            // check default domain via Resolver before
            const defaultDomainViaResolverBefore = await situsResolverNonUpgradable.getFirstDefaultDomain(user1.address);
            expect(defaultDomainViaResolverBefore).to.equal("");
            ...
            // check default domain via Resolver after
            const defaultDomainViaResolverAfter = await situsResolverNonUpgradable.getFirstDefaultDomain(user1.address);
            expect(defaultDomainViaResolverAfter).to.equal(domainName + tldName1);
        
            // check non-existing default domain via Resolver
            const defaultDomainViaResolverNonExisting = await situsResolverNonUpgradable.getFirstDefaultDomain(user2.address);
            expect(defaultDomainViaResolverNonExisting).to.equal("");
          ...
          it('un/sets a TLD as deprecated (only owner)', async function () {
            ...
            await expect(
              situsResolverNonUpgradable.connect(user1).addDeprecatedTldAddress(tldContract2.getAddress())
            ).to.be.revertedWith('Ownable: caller is not the owner');
          });
          ...
          it('allows user to set a custom default domain', async function () {
            ...
            const defaultFirstDomainViaResolverBefore = await situsResolverNonUpgradable.getFirstDefaultDomain(user1.address);
            ...
            await situsResolverNonUpgradable.connect(user1).setCustomDefaultDomain(domainName2, ".wagmi");
        
            // check the first default domain via Resolver after
            const defaultFirstDomainViaResolverAfter = await situsResolverNonUpgradable.getFirstDefaultDomain(user1.address);
            expect(defaultFirstDomainViaResolverAfter).to.equal(domainName2+".wagmi");
        
            await expect(situsResolverNonUpgradable.connect(user1).setCustomDefaultDomain("admin", ".wagmi")).to.be.revertedWith('You do not own this domain.');;
          
            // un-set/remove a custom default domain
            await situsResolverNonUpgradable.connect(user1).setCustomDefaultDomain("", "");
        
            // check the first default domain via Resolver after
            const defaultFirstDomainViaResolverAfter2 = await situsResolverNonUpgradable.getFirstDefaultDomain(user1.address);
